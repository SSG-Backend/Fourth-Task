...
app.post("/", async (request, response) => {
  ...
  console.log(`Upload of ${files.images.name} received!`);

  await knex.insert({
    name: files.images.name,
    image: files.images.data //The actual binary buffer
  }).into("images");

  files.photograph.mv(`./uploads/${files.images.name}`, async (error) => {
    if (error) {
